...
export class Aragon {
    ...
    async ambilData() {
        ...
        for (let i = 2; i < rowCount; i++) {
            ...
            const data = {
                ...
                jumlah_charter  : Number((await col.nth(6).innerText()).trim()),
                penjualan_charter   : this.parseRupiah(await col.nth(7).innerText()),
                op_bbm          : this.parseRupiah(await col.nth(8).innerText()),
                op_tol          : this.parseRupiah(await col.nth(9).innerText()),
                op_driver       : this.parseRupiah(await col.nth(10).innerText()),
                op_tol_tambahan : this.parseRupiah(await col.nth(11).innerText()),
    ...
    async ambilDataTotal() {
        ...
        const result =  {
            ...
            total_jumlah_charter    : Number((await col_total.nth(6).innerText()).trim()),
            total_penjualan_charter : this.parseRupiah(await col_total.nth(7).innerText()),
            total_op_bbm            : this.parseRupiah(await col_total.nth(8).innerText()),
            total_op_tol            : this.parseRupiah(await col_total.nth(9).innerText()),
            total_op_driver         : this.parseRupiah(await col_total.nth(10).innerText()),
            total_op_tol_tambahan   : this.parseRupiah(await col_total.nth(11).innerText()),
    ...
    async hitungPengeluaran(values) {
        ...
        for (const value of values) {
            ...
            const data = {
                id_tanggal  : await value.tanggal,
                pengeluaran : await value.op_bbm + value.op_tol + value.op_driver + value.op_tol_tambahan
            }
    ...
    async hitungTotalPerField(values) {
        ...
        let temp_total_op_tol_tambahan   = 0;
        ...
        for (const value of values) {
            ...
            temp_total_op_tol_tambahan = temp_total_op_tol_tambahan + value.op_tol_tambahan;   
            ...
            result = {
                ...
                total_op_tol_tambahan : temp_total_op_tol_tambahan,
