...
export class Baraya {
    ...
    async ambilData() {
        ...
        for (let i = 2; i < rowCount; i++) {
            ...
            const data = {
                ...
                op_sopir        : this.parseRupiah(await col.nth(7).innerText()),
                op_sopir_tambahan   :this.parseRupiah(await col.nth(8).innerText()) ,
                op_bbm_tambahan : this.parseRupiah(await col.nth(9).innerText()),
                op_lain         : this.parseRupiah(await col.nth(10).innerText()),
                op_parkir       : this.parseRupiah(await col.nth(11).innerText()),
                op_tol          : this.parseRupiah(await col.nth(12).innerText()),
                op_total        : this.parseRupiah(await col.nth(13).innerText()),
                laba            : this.parseRupiah(await col.nth(14).innerText())
            };
    ...
    async ambilDataTotal() {
        ...
        const result =  {
            ...
            total_op_sopir        : this.parseRupiah(await col_total.nth(7).innerText()),
            total_op_sopir_tambahan   :this.parseRupiah(await col_total.nth(8).innerText()) ,
            total_op_bbm_tambahan : this.parseRupiah(await col_total.nth(9).innerText()),
            total_op_lain         : this.parseRupiah(await col_total.nth(10).innerText()),
            total_op_parkir       : this.parseRupiah(await col_total.nth(11).innerText()),
            total_op_tol          : this.parseRupiah(await col_total.nth(12).innerText()),
    ...
    async hitungPengeluaran(values) {
        ...
        for (const value of values) {
            ...
            const data = {
                id_tanggal  : await value.tanggal,
                pengeluaran : await value.op_bbm + value.op_sopir + value.op_sopir_tambahan + value.op_bbm_tambahan + value.op_lain + value.op_parkir + value.op_tol
            }
    ...
    async hitungTotalPerField(values) {
        ...
        let temp_total_op_sopir        = 0;
        let temp_total_op_sopir_tambahan    = 0;        
        let temp_total_op_bbm_tambahan = 0;
        let temp_total_op_lain         = 0;
        let temp_total_op_parkir       = 0;
        ...
        for (const value of values) {
            ...
            temp_total_op_sopir        = temp_total_op_sopir + value.op_sopir;
            temp_total_op_sopir_tambahan    = temp_total_op_sopir_tambahan + value.op_sopir_tambahan;        
            temp_total_op_bbm_tambahan = temp_total_op_bbm_tambahan + value.op_bbm_tambahan;
            temp_total_op_lain         = temp_total_op_lain + value.op_lain;
            temp_total_op_parkir       = temp_total_op_parkir + value.op_parkir;
            ...
            result = {
                ...
                total_op_sopir          : temp_total_op_sopir,
                total_op_sopir_tambahan : temp_total_op_sopir_tambahan,           
                total_op_bbm_tambahan   : temp_total_op_bbm_tambahan,
                total_op_lain           : temp_total_op_lain,       
                total_op_parkir         : temp_total_op_parkir,    
